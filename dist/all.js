	...
	function CNFlexFormService(Api, $parse, cnFlexFormConfig, cnFlexFormTypes, sfPath, $interpolate, $timeout, cnUtil, $stateParams, EVENTS) {
	  ...
	  function processField(field, pos) {
	    ...
	    if (!_.isUndefined(field.arrayIndex)) {
	      var _key2 = service.getKey(field.key);
	      _key2 = _key2.replace('[]', '');
	      _.each(service.schema.data, function (val, prop) {
	        if (prop.includes(_key2)) {
	          var diffArr = _.difference(prop.split('.'), _key2.split('.'));
	          if (diffArr.length) {
	            if (field.items) {
	              _.each(field.items, function (item) {
	                service.parseStringKey(item, diffArr.filter(function (d) {
	      ...
	      service.scope.$broadcast('cnFlexFormReprocessField', _key2);
	    }
	  }
